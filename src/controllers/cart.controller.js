...
const createCart = async(req,res)=> {
    try {
        const {user,product, inventory, quality} = req.body

        if ([user, product,  inventory,  quality ].some((field)=> field === ""))
        {
        ...
        if(isCart){
            const cart = await Cart.findByIdAndUpdate({_id: isCart._id},{
...
const updateQuantity = async (req,res)=> {
    try {
        ...
        if ([user,  inventory,  value ].some((field)=> field === ""))
            ...
            if(value === "plus") {
                const cart = await Cart.findOneAndUpdate({user, inventory},{
                    $inc: {quantity:1} }, {new: true})
                    return res.json(apiResponse(201, "cart updated", { cart }))
            } else if(value === "minus" ){
                const cart = await Cart.findOneAndUpdate({user, inventory}, {
